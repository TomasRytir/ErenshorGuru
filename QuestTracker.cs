...
namespace ErenshorQuestGuru
{
    ...
    public class QuestTracker : MonoBehaviour
    {
        ...
        private int trackedQuestID = -1;  // Track quest by ID
        ...
        // Helper method to find quest by ID in cached quests
        public QuestData GetQuestByID(int questID)
        {
            return cachedActiveQuests.FirstOrDefault(q => q.QuestID == questID);
        }
        ...
        private void RefreshQuestCache()
        {
            try
            {
                ...
                // IMPORTANT: Only update cache if we got quests OR if quest was completed
                // If newQuests is empty but we had quests, journal might just be closed
                if (newQuests.Count > 0)
                {
                    ...
                    if (newQuests.Count != lastQuestCount && lastQuestCount != -1)
                    {
                        ...
                        if (Plugin.GetTrackedQuest() != null)
                        {
                            ...
                            if (!stillExists)
                            {
                                QuestGuruPlugin.Logger.LogInfo($"Tracked quest '{trackedQuest.Name}' completed - untracking");
                                Plugin.UntrackQuest();
                            }
                        }
                    }
                ...
                else if (cachedActiveQuests.Count > 0)
                {
                    // Empty response but we have cache - journal is probably closed, keep cache
                    // Don't log spam
                }
            }
            catch (Exception ex)
        ...
        private void DrawTrackerWindow(int windowID)
        {
            ...
            // Get tracked quest from our cache
            QuestData trackedQuest = trackedQuestID != -1 ? GetQuestByID(trackedQuestID) : null;
        ...
        private void DrawQuestListWindow(int windowID)
        {
            ...
            else
            {
                ...
                {
                    bool isTracked = trackedQuestID == quest.QuestID;
